...
class BusinessService implements BusinessServiceInterface{
  ...
  @override
  Future<String> processesBusinessPlan(String businessPlanStatus, int paymentIndex, int storeId, PackageModel? packageModel, String? digitalPaymentName, int activeSubscriptionIndex) async {
  ...
  Future<String> setUpBusinessPlan(BusinessPlanBody businessPlanBody, String? digitalPaymentName, String businessPlanStatus, int storeId) async {
    ...
    if (response.statusCode == 200) {
      if(response.body['redirect_link'] != null) {
        ...
        Get.back();
        Get.toNamed(RouteHelper.getPaymentRoute(digitalPaymentName, redirectUrl, storeId, true));
      }else {
        businessPlanStatus = 'complete';
        Get.offAllNamed(RouteHelper.getSubscriptionSuccessRoute(status: 'success', fromSubscription: false, storeId: storeId));
      }
    }
    return businessPlanStatus;
  }
